...
namespace static_tuple_adl
{
	...
	public:
		...
		constexpr explicit(false) static_tuple(auto&& first, auto&&... rest) noexcept
			: base_type(NGS_STL_FREESTANDING_UTILITY_FORWARD(rest)...)
			, _data(NGS_STL_FREESTANDING_UTILITY_FORWARD(first))
		{
		}
		...
		requires (I < tuple_size_v<::std::remove_cvref_t<decltype(p)>>)
	{
		...
		{
			return NGS_STL_FREESTANDING_UTILITY_FORWARD(p)._value();
		}
		else
		{
			return static_tuple_adl::get_impl<I>(NGS_STL_FREESTANDING_UTILITY_FORWARD(p)._base());
		}
	}

	template<::std::size_t I, class... Args>
	constexpr decltype(auto) get(static_tuple<Args...>& t) { return static_tuple_adl::get_impl<sizeof...(Args) - 1 - I>(NGS_STL_FREESTANDING_UTILITY_FORWARD(t)); }
	template<::std::size_t I, class... Args>
	constexpr decltype(auto) get(const static_tuple<Args...>& t) { return static_tuple_adl::get_impl<sizeof...(Args) - 1 - I>(NGS_STL_FREESTANDING_UTILITY_FORWARD(t)); }
	template<::std::size_t I, class... Args>
	constexpr decltype(auto) get(static_tuple<Args...>&& t) { return static_tuple_adl::get_impl<sizeof...(Args) - 1 - I>(NGS_STL_FREESTANDING_UTILITY_FORWARD(t)); }
	template<::std::size_t I, class... Args>
	constexpr decltype(auto) get(const static_tuple<Args...>&& t) { return static_tuple_adl::get_impl<sizeof...(Args) - 1 - I>(NGS_STL_FREESTANDING_UTILITY_FORWARD(t)); }
